...
std::string proxyToSurge(std::vector<Proxy> &nodes, const std::string &base_conf, std::vector<RulesetContent> &ruleset_content_array, const ProxyGroupConfigs &extra_proxy_group, int surge_ver, extra_settings &ext)
{
    ...
    for(Proxy &x : nodes)
    {
        ...
        {
        case ProxyType::Shadowsocks:
            if(surge_ver >= 3 || surge_ver == -3 || shadowrocket)
            {
            ...
            break;
        case ProxyType::VMess:
            if(surge_ver < 4 && surge_ver != -3 && !shadowrocket)
                continue;
        ...
        case ProxyType::Trojan:
            ...
            if(shadowrocket && !host.empty())
                proxy += ", peer=" + host;
            else if(!host.empty())
                proxy += ", sni=" + host;
            if(shadowrocket && scv.get())
                proxy += ", allowInsecure=1";
        ...
        case ProxyType::WireGuard:
            ...
            break;
        case ProxyType::Hysteria2:
            if(surge_ver < 4 && !shadowrocket)
                continue;
            if(shadowrocket)
            {
                proxy = "hysteria2, " + hostname + ", " + port + ", auth=" + password;
                if(!obfsparam.empty())
                    proxy += ", obfsParam=" + obfsparam;
                if(!x.SNI.empty())
                    proxy += ", peer=" + x.SNI;
                if(!x.Alpn.empty())
                    proxy += ", alpn=" + join(x.Alpn, "|");
                if(x.UpSpeed)
                    proxy += ", upmbps=" + std::to_string(x.UpSpeed);
                if(x.DownSpeed)
                    proxy += ", downmbps=" + std::to_string(x.DownSpeed);
            }
            else
            {
                ...
                if(!x.Fingerprint.empty())
                {
                    auto fp = toLower(x.Fingerprint);
                    if(fp != "chrome" && fp != "firefox" && fp != "safari" && fp != "ios" && fp != "android" && fp != "edge" && fp != "360" && fp != "qq" && fp != "random" && fp != "randomized")
        ...
        {
            if(!tfo.is_undef())
                proxy += ", tfo=" + std::string(tfo.get() ? "1" : "0");
            if(!udp.is_undef())
                proxy += ", udp=" + std::string(udp.get() ? "1" : "0");
        }
        else
        {
            if(!tfo.is_undef())
...
{
    /// types: SS=1 SSR=2 VMess=4 Trojan=8 Hysteria2=16
    std::string proxyStr, allLinks;
    bool ss = GETBIT(types, 1), ssr = GETBIT(types, 2), vmess = GETBIT(types, 3), trojan = GETBIT(types, 4), hysteria2 = GETBIT(types, 5);
    ...
    for(Proxy &x : nodes)
    {
        ...
        case ProxyType::Hysteria2:
                ...
                continue;
            {
                ...
                proxyStr = "hysteria2://" + urlEncode(password) + "@" + hostname + ":" + port;
                if(!x.Ports.empty() && x.Ports != port)
                    args.emplace_back("mport=" + urlEncode(x.Ports));
                if(!obfs.empty())
                    args.emplace_back("obfs=" + urlEncode(obfs));
                if(!obfsparam.empty())
                    args.emplace_back("obfs-password=" + urlEncode(obfsparam));
                if(!x.SNI.empty())
                    args.emplace_back("sni=" + urlEncode(x.SNI));
                if(!scv.is_undef())
                    args.emplace_back("insecure=" + std::string(scv.get() ? "1" : "0"));
                if(!x.Fingerprint.empty())
                {
                    auto fp = toLower(x.Fingerprint);
                    if(fp == "chrome" || fp == "firefox" || fp == "safari" || fp == "ios" || fp == "android" || fp == "edge" || fp == "360" || fp == "qq" || fp == "random" || fp == "randomized")
                        args.emplace_back("fp=" + urlEncode(x.Fingerprint));
                    else
                        args.emplace_back("pinSHA256=" + urlEncode(x.Fingerprint));
                }
                if(!x.Alpn.empty())
                    args.emplace_back("alpn=" + urlEncode(join(x.Alpn, ",")));
                if(!args.empty())
                    proxyStr += "/?" + join(args, "&");
                proxyStr += "#" + urlEncode(remark);
            }
