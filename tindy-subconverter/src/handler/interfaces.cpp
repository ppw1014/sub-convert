...
std::string subconverter(RESPONSE_CALLBACK_ARGS)
{
    ...
    case "ss"_hash: case "ssd"_hash: case "ssr"_hash: case "sssub"_hash: case "v2ray"_hash: case "trojan"_hash: case "mixed"_hash:
        ...
        break;
    case "clash"_hash: case "clashr"_hash: case "surge"_hash: case "shadowrocket"_hash: case "quan"_hash: case "quanx"_hash: case "loon"_hash: case "surfboard"_hash: case "mellow"_hash: case "singbox"_hash:
        break;
    default:
        *status_code = 400;
...
//    }
    ...
    */
    ...
    case "shadowrocket"_hash:
        writeLog(0, "Generate target: Shadowrocket", LOG_LEVEL_INFO);

        {
            auto shadowrocket_tpl_args = tpl_args;
            shadowrocket_tpl_args.request_params["target"] = "surge";
            if(render_template(fetchFile(lSurgeBase, proxy, global.cacheConfig), shadowrocket_tpl_args, base_content, global.templatePath) != 0)
            {
                *status_code = 400;
                return base_content;
            }
        }
        output_content = proxyToSurge(nodes, base_content, lRulesetContent, lCustomProxyGroups, -5, ext);

        if(argUpload)
            uploadGist("shadowrocket", argUploadPath, output_content, true);
        break;
