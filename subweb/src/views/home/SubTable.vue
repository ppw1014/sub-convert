...
export default {
  ...
  data() {
    const defaultApiUrl = window.config.apiUrl || window.location.origin;
    return {
      placeholder: '多订阅链接或节点请确保每行一条\n支持手动使用"|"分割多链接或节点',
      targetOptions: [
        { value: 'clash', text: 'Clash' },
        { value: 'clashr', text: 'ClashR' },
        { value: 'mixed', text: 'Shadowrocket / Mixed' },
        { value: 'shadowrocket', text: 'Shadowrocket 配置' },
  ...
  methods: {
    ...
    selectTarget(event) {
      if (event.target.value == 'shadowrocket') {
        this.remoteConfig = 'config/loyalsoldier_shadowrocket.ini';
        this.isShowRemoteConfig = false;
      } else if (this.remoteConfig == 'config/loyalsoldier_shadowrocket.ini') {
        this.remoteConfig = window.config.defaultRemoteConfig || '';
      }
    },
    selectApi(event) {
